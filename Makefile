...
# kind-test: kind-test-install
	...
	kubectl -n bashbot get pods | grep bashbot 
	kubectl -n bashbot get pods | grep bashbot | cut -d' ' -f1
	$(eval PODNAME=$(shell sh -c "kubectl -n bashbot get pods | grep bashbot | cut -d' ' -f1"))
	echo "PODNAME: $(shell kubectl -n bashbot get pods | grep bashbot | cut -d' ' -f1)"
	kubectl --namespace bashbot exec $(shell kubectl -n bashbot get pods | grep bashbot | cut -d' ' -f1) -- bash -c \
...
# --set 'image.command={/bin/bash}' \
# --set 'image.args={-c,echo \"hello\" && sleep 3600}'
	$(eval export PODNAME=$(shell sh -c "kubectl -n bashbot get pods | grep bashbot | cut -d' ' -f1"))
